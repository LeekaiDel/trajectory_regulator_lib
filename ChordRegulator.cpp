#include <regulator/ChordRegulator.h>
...
Eigen::Vector3d ChordRegulator::GetLeadingVector()
{
    ...
    if (ChordRegulator::chords_vector.size() > 0)
    {
        ...
        ChordRegulator::full_length_path = 0.0;
        ChordRegulator::full_length_path = ChordRegulator::dist_to_chord + (chord[1] - pose_projection_on_chord).norm();
        for (int i = ChordRegulator::chord_list_id; i < ChordRegulator::chords_vector.size() - 1; ++i)
        {
            ChordRegulator::full_length_path += (ChordRegulator::chords_vector[i + 1][1] - ChordRegulator::chords_vector[i + 1][0]).norm();
        }

        Eigen::Vector3d local_pose_projection_on_chord = pose_projection_on_chord - ChordRegulator::robot_pose;
        ...
        {
            // double length = (local_chord[1] - local_pose_projection_on_chord).norm();
            // if (length > 5)
            // {
            //     Eigen::Vector3d vector_on_end_line = (local_chord[1] - local_pose_projection_on_chord) * 5 / length;
            //     vector_on_end_line = vector_on_end_line + local_pose_projection_on_chord;

            //     float k = ChordRegulator::dist_to_chord / ChordRegulator::min_dist_to_chord;
            //     leading_vector = vector_on_end_line - local_pose_projection_on_chord + k * (local_pose_projection_on_chord - (vector_on_end_line - local_pose_projection_on_chord)); //- (local_chord[1] - local_pose_projection_on_chord)
            // }
            // else
            // {
                float k = ChordRegulator::dist_to_chord / ChordRegulator::min_dist_to_chord;
                leading_vector = local_chord[1] - local_pose_projection_on_chord + k * (local_pose_projection_on_chord - (local_chord[1] - local_pose_projection_on_chord)); //- (local_chord[1] - local_pose_projection_on_chord)
            // }
            // float k = ChordRegulator::dist_to_chord / ChordRegulator::min_dist_to_chord;
            // leading_vector = local_chord[1] - local_pose_projection_on_chord + k * (local_pose_projection_on_chord - (local_chord[1] - local_pose_projection_on_chord)); //- (local_chord[1] - local_pose_projection_on_chord)
        }
        ...
        if ((pose_projection_on_chord - chord[1]).norm() < ChordRegulator::min_dist_to_chord && ChordRegulator::chord_list_id < ChordRegulator::chords_vector.size() - 1)
        {
            ChordRegulator::chord_list_id += 1;
        }
        else if (ChordRegulator::chord_list_id >= ChordRegulator::chords_vector.size() - 1 && local_chord[1].norm() < ChordRegulator::min_dist_to_chord)
        {
            ChordRegulator::up_index = 1;
        }

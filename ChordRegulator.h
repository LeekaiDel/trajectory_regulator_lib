#ifndef CHORD_REGULATOR_H
#define CHORD_REGULATOR_H
...
class ChordRegulator
{
    public:
        ...
        float min_dist_to_chord = 4;                              // Расстояние срабатывания отклонения направляющего вектора при следовании к траектории
        float max_velocity = 2.5;
        // Глобальные переменные
        float full_length_path = 0.0;
        int up_index = 0;
        ...
        std::vector<std::vector<Eigen::Vector3d>> chords_vector;    // Вектор состоящий из прямых имеющих начало и конец
        // Конструктор
        ChordRegulator();
